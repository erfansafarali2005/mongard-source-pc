...
class UserLogoutView(LoginRequiredMixin , View):
    def get(self , request):
        logout(request)
        messages.success(request , 'you loggged out successfully' , 'success')
        return redirect("home:home")
        ...
        if form.is_valid():
            try :
                ...
                if user is not None:
                    ...
                    messages.error(request , 'wrong credentials' , 'danger')
                    return redirect('account:user_login')
            except :
                messages.error(request , 'there was an error in loging ' , 'error')
